...
const Group = db.get<IGroup>('groups');

Group.createIndex('createdAt');
Group.createIndex('updatedAt');
Group.createIndex('lastActivityAt');
Group.createIndex('followersCount');
Group.createIndex('tags');
Group.createIndex('isSuspended');
Group.createIndex('username');
Group.createIndex('usernameLower');
Group.createIndex('host');
Group.createIndex(['username', 'host'], { unique: true });
Group.createIndex(['usernameLower', 'host'], { unique: true });
Group.createIndex('token', { sparse: true, unique: true });
Group.createIndex('uri', { sparse: true, unique: true });
...
export async function pack(
	src: IGroup,
	me?: string | mongo.ObjectID | IGroup | null,
	options?: PackOptions
): Promise<PackedUser>;
export async function pack(
 ...
 */
export async function pack(
	src: string | mongo.ObjectID | IGroup,
	me?: string | mongo.ObjectID | IGroup | null,
	options?: PackOptions
): Promise<PackedUser | null> {
	...
	let db: IGroup | undefined;

	// Populate the group if 'group' is ID
	if (isObjectId(src)) {
		db = await Group.findOne({
			_id: src
		});
	...
	if (db == null) {
		dbLogger.warn(`group not found on database: ${src}`);
		return null;
	}
